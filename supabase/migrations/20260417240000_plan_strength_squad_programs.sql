-- Planned strength: create one workout per person (host + participants) with distinct programs,
-- and add every squad member as workout_participant on each workout so dual/group pickers work.
-- Caller must be the first program owner (host).
...
REVOKE ALL ON FUNCTION public.plan_strength_squad_programs(jsonb, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.plan_strength_squad_programs(jsonb, text, text, text, text) TO authenticated;
