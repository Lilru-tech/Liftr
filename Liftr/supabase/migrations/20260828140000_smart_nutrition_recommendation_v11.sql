...
create or replace function public.nutrition_rank_insights_v1(
  p_insights jsonb,
  p_max_cards integer default 8
)
returns jsonb
language plpgsql
immutable
...
declare
  ...
  v_max_warnings integer := 4;
  v_max_positive integer := 2;
begin
  select coalesce(jsonb_agg(x order by (x->>'priority')::int desc), '[]'::jsonb)
  into v_positive
  from jsonb_array_elements(coalesce(p_insights, '[]'::jsonb)) x
  where x->>'sentiment' = 'positive';

  select coalesce(jsonb_agg(x order by (x->>'priority')::int desc), '[]'::jsonb)
  into v_warning
  from jsonb_array_elements(coalesce(p_insights, '[]'::jsonb)) x
  where x->>'sentiment' = 'warning';

  select coalesce(jsonb_agg(x order by (x->>'priority')::int desc), '[]'::jsonb)
  into v_info
  from jsonb_array_elements(coalesce(p_insights, '[]'::jsonb)) x
  where x->>'sentiment' = 'info';

  v_result := v_result || coalesce((
    select jsonb_agg(x)
    from (
      select x
      from jsonb_array_elements(v_warning) x
      limit v_max_warnings
    ) w
  ), '[]'::jsonb);
  ...
  v_result := v_result || coalesce((
    ...
    from (
      select x
      from jsonb_array_elements(v_positive) x
      limit v_max_positive
    ) p
  ), '[]'::jsonb);
  ...
  v_result := v_result || coalesce((
    ...
    from (
      select x
      from jsonb_array_elements(v_info) x
      limit greatest(0, p_max_cards - jsonb_array_length(v_result))
    ) i
  ), '[]'::jsonb);
...
create or replace function public.get_smart_nutrition_recommendation_v2(
  p_start_date date,
  p_end_date date
)
returns jsonb
...
begin
  ...
  return public.nutrition_smart_recommendation_compute_v2(v_uid, p_start_date, p_end_date);
end;
$$;

create or replace function public.get_daily_nutrition_insights_v1(
...
begin
  ...
  into v_day_kcal
  from public.nutrition_log_metrics_v1(v_uid, p_date, p_date);

  with daily as (
    select log_date, sum(kcal) as day_kcal
    from public.nutrition_log_metrics_v1(v_uid, p_date - 14, p_date - 1)
    group by log_date
  )
  select coalesce(percentile_cont(0.5) within group (order by day_kcal), 0)
  into v_median_kcal
  from daily;

  select coalesce((public.get_daily_nutrition_recommendation_v1(p_date)->>'remaining_calories')::numeric, 0)
  into v_remaining;

  if v_day_kcal <= 0 then
    v_text := 'Log your meals today to unlock personalized nutrition insights.';
    return jsonb_build_object(
      'recommendation_text', v_text,
      'insights', '[]'::jsonb,
      'remaining_calories', v_remaining
    );
  end if;

  if v_median_kcal > 0 and v_day_kcal < v_median_kcal * 0.4 and p_date >= current_date - 1 then
    v_insights := jsonb_build_array(jsonb_build_object(
      'id', 'partial_day_today',
      'category', 'logging_quality',
      'sentiment', 'info',
      'priority', 85,
      'title', 'Today looks incomplete',
      'body', format(
        'You have logged %s kcal so far today, well below your recent typical day (~%s kcal). Finish logging or remove partial entries to keep advice accurate.',
        round(v_day_kcal, 0)::text,
        round(v_median_kcal, 0)::text
      ),
      'metadata', jsonb_build_object('day_kcal', round(v_day_kcal, 1), 'median_kcal', round(v_median_kcal, 1))
    ));
    v_text := v_insights->0->>'body';
  elsif v_remaining < -400 then
    v_insights := jsonb_build_array(jsonb_build_object(
      'id', 'over_budget_today',
      'category', 'energy_balance',
      'sentiment', 'warning',
      'priority', 80,
      'title', 'Over today''s energy budget',
      'body', format(
        'You are %s kcal over your daily budget after activity. Keep remaining meals lighter on calories and prioritize protein.',
        round(abs(v_remaining), 0)::text
      ),
      'metadata', jsonb_build_object('remaining_kcal', round(v_remaining, 1))
    ));
    v_text := v_insights->0->>'body';
  elsif v_remaining > 400 then
    v_insights := jsonb_build_array(jsonb_build_object(
      'id', 'budget_left_today',
      'category', 'energy_balance',
      'sentiment', 'info',
      'priority', 70,
      'title', 'Room left in today''s budget',
      'body', format(
        'You still have about %s kcal in today''s energy budget. A protein-rich snack or post-workout meal can use that room productively.',
    ...
    v_rec := public.nutrition_smart_recommendation_compute_v2(v_uid, p_date - 6, p_date);
    if jsonb_array_length(coalesce(v_rec->'insights', '[]'::jsonb)) > 0 then
      v_insights := jsonb_build_array(v_rec->'insights'->0);
      v_text := v_rec->'insights'->0->>'body';
    else
      v_text := coalesce(v_rec->>'recommendation_text', 'Keep logging consistently to unlock deeper coaching insights.');
    end if;
  end if;
  ...
  return jsonb_build_object(
    ...
    'day_kcal', round(v_day_kcal, 1)
  );
end;
$$;

revoke all on function public.nutrition_rank_insights_v1(jsonb, integer) from public;
revoke all on function public.nutrition_smart_recommendation_compute_v2(uuid, date, date) from public;
revoke all on function public.get_smart_nutrition_recommendation_v2(date, date) from public;
grant execute on function public.get_smart_nutrition_recommendation_v2(date, date) to authenticated;
revoke all on function public.get_daily_nutrition_insights_v1(date) from public;
grant execute on function public.get_daily_nutrition_insights_v1(date) to authenticated;
