...
create or replace function public.liftr_combat_handicap_health_weight_v1()
returns numeric
language sql
immutable
as $$
...
create or replace function public.liftr_combat_handicap_threshold_pct_v1()
returns integer
language sql
immutable
as $$
...
create or replace function public.liftr_combat_comparison_pool_v1(
  p_health integer,
  p_strength integer,
  p_defense integer,
  p_speed integer,
...
as $$
  select
    floor(coalesce(p_health, 0) * public.liftr_combat_handicap_health_weight_v1())::integer
...
as $$
  select
    ...
    > least(coalesce(p_pool_a, 0), coalesce(p_pool_b, 0)) * public.liftr_combat_handicap_threshold_pct_v1();
$$;
