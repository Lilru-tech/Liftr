...
create or replace function public._liftr_strength_edit_save_active()
returns boolean
language sql
stable
set search_path to 'public'
as $function$
  select coalesce(nullif(current_setting('liftr.strength_edit_save', true), ''), 'off') = 'on';
$function$;
...
as $function$
begin
  if public._liftr_strength_edit_save_active()
     or public._liftr_bulk_strength_replace_active()
  then
    return NEW;
  end if;
