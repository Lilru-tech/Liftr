...
create or replace function public.liftr_combat_loser_rewards(p_winner_rewards jsonb)
returns jsonb
language plpgsql
immutable
as $$
declare
  ...
  v_coins integer;
begin
  v_winner_xp := coalesce((p_winner_rewards->>'xp')::integer, 0);
  v_winner_coins := coalesce((p_winner_rewards->>'coins')::integer, 0);
  v_xp := greatest(3, floor(v_winner_xp * 0.25)::integer);
  v_coins := greatest(1, floor(v_winner_coins * 0.25)::integer);
...
revoke all on function public.liftr_combat_loser_rewards(jsonb) from public;
