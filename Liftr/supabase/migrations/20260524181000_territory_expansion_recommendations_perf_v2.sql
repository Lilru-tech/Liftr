...
begin
  ...
  user_cells_frontier as (
    select uc.cell_id, uc.cell_geog, uc.city_key, uc.dist_m
    from user_cells uc
  ...
  city_owned as (
    select tc.city_key, tc.owner_user_id, count(*)::integer as owned_cells
    from public.territory_cells tc
  ...
  p2_candidates as (
    ...
    from user_cells_frontier uc
      inner join public.territory_cells tc2
        ...
        and st_touches(uc.cell_geog::geometry, tc2.cell_geog::geometry)
    where tc2.owner_user_id <> p_user_id
      and st_dwithin(tc2.cell_geog, v_user_point, v_radius)
    order by tc2.cell_id
  ),
  combined as (
    select p1.cell_id, p1.cell_geog, p1.weight_priority, p1.leader_sort, p1.dist_m from p1_candidates p1
    union all
    select p2.cell_id, p2.cell_geog, p2.weight_priority, p2.leader_sort, p2.dist_m from p2_candidates p2
  ),
  deduped as (
    select distinct on (c.cell_id) c.cell_id, c.cell_geog, c.weight_priority
    from combined c
  ...
  select
    ...
    d.weight_priority
  from deduped d
  order by d.weight_priority asc,
