  ...
  lm as (
    select distinct on (m.conversation_id)
      ...
      left(
        case
          when nullif(
            trim(both from regexp_replace(coalesce(m.body, ''), '[[:space:]]+', ' ', 'g')),
            ''
          ) is not null then
            trim(both from regexp_replace(coalesce(m.body, ''), '[[:space:]]+', ' ', 'g'))
          ...
          when m.kind = 'shared_ingredient' then
            coalesce(nullif(trim(both from m.metadata->>'name'), ''), 'Shared ingredient')
          when m.kind = 'shared_recipe' then
            coalesce(nullif(trim(both from m.metadata->>'name'), ''), 'Shared recipe')
...
begin
  ...
  v_preview := case
    ...
    when new.kind = 'workout_share' then
      coalesce(
        nullif(left(regexp_replace(coalesce(new.body, ''), '[[:space:]]+', ' ', 'g'), 140), ''),
        'Sent a workout'
      )
