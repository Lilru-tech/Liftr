...
alter table public.pet_combat_history
  add column if not exists is_handicapped boolean not null default false;

create index if not exists pet_combat_history_h2h_competitive_idx
  on public.pet_combat_history (attacker_user_id, defender_user_id, created_at desc)
  where is_handicapped = false;

create or replace function public.liftr_combat_hardcore_rewards(
  p_attacker_level integer,
  p_defender_level integer,
  p_stronger_pool integer,
...
declare
  ...
  v_coins integer;
begin
  v_premium := public.liftr_combat_premium_rewards(p_attacker_level, p_defender_level);
  v_buff_multiplier := greatest(p_stronger_pool, 1)::numeric / greatest(p_weaker_pool, 1)::numeric;
  v_xp := greatest(10, floor((v_premium->>'xp')::integer * v_buff_multiplier)::integer);
  v_coins := greatest(5, floor((v_premium->>'coins')::integer * v_buff_multiplier)::integer);
...
begin
  ...
  where (
      (h.attacker_user_id = v_me and h.defender_user_id = p_opponent_user_id)
      or (h.attacker_user_id = p_opponent_user_id and h.defender_user_id = v_me)
    )
    and coalesce(h.is_handicapped, false) = false;
...
begin
  ...
  if v_is_handicapped then
    v_hardcore_buff_multiplier := greatest(v_attacker_pool_original, v_defender_pool_original)::numeric
      / greatest(least(v_attacker_pool_original, v_defender_pool_original), 1)::numeric;

    v_disable_nerf_effective := coalesce(p_disable_nerf_choice, false)
      and v_attacker_pool_original < v_defender_pool_original;
...
revoke all on function public.liftr_combat_hardcore_rewards(integer, integer, integer, integer) from public;
revoke all on function public.get_pet_combat_head_to_head_v1(uuid) from public;
