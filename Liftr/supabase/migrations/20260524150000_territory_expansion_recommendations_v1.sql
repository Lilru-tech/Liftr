...
begin
  ...
  neighbor_raw as (
    select distinct on (nh.cell_id)
      nh.cell_id,
      coalesce(tc.cell_geog, nh.cell_geog) as cell_geog,
      st_distance(
        st_centroid(coalesce(tc.cell_geog, nh.cell_geog)::geometry),
    ...
    where nh.cell_id <> uc.cell_id
      ...
      and st_dwithin(coalesce(tc.cell_geog, nh.cell_geog), v_user_point, v_radius)
      and (tc.cell_id is null or tc.owner_user_id <> p_user_id)
    order by nh.cell_id
  ),
  p2_candidates as (
    select
