...
create or replace function public.nutrition_demographic_fallback_kcal_v1(p_sex text)
returns integer
language plpgsql
stable
...
declare
  v_sex text;
begin
  if p_sex is null or btrim(p_sex) = '' then
    return 1700;
  end if;

  v_sex := lower(btrim(p_sex));

  if v_sex in ('female', 'f') then
    return 1500;
  elsif v_sex in ('male', 'm') then
    return 1900;
  end if;
...
begin
  ...
  return greatest(800, least(6000, round(v_bmr * 1.2)::integer));
end;
$$;
        ...
        greatest(
          ...
          least(
            ...
            coalesce(
              ...
              public.nutrition_demographic_fallback_kcal_v1(p.sex::text)
            )
          )
        )
      else
...
revoke all on function public.nutrition_demographic_fallback_kcal_v1(text) from public;
grant execute on function public.nutrition_demographic_fallback_kcal_v1(text) to authenticated;

revoke all on function public.nutrition_compute_metabolic_target_kcal_v1(text, date, numeric, numeric) from public;
grant execute on function public.nutrition_compute_metabolic_target_kcal_v1(text, date, numeric, numeric) to authenticated;
