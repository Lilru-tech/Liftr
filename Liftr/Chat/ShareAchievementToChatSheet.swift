...
struct ShareAchievementToChatSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Send achievement")
        ...
        if model.loading && model.rows.isEmpty {
            ...
            VStack(spacing: 8) {
                ...
                Text("No conversations yet")
                    .font(.headline)
                Text("Tap + to start a chat, then send this achievement.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .multilineTextAlignment(.center)
            }
    ...
    private func send(to conversationId: Int64) async {
        ...
        do {
            _ = try await ChatService.sendAchievementShare(conversationId: conversationId, snapshot: snapshot)
            onSent()
            dismiss()
        } catch {
    ...
    private func sendToNew(profile: ChatProfile) async {
        do {
            ...
            _ = try await ChatService.sendAchievementShare(conversationId: cid, snapshot: snapshot)
            await MainActor.run {
                showNewChat = false
                onSent()
                dismiss()
