...
@MainActor
final class ShareRecipeToChatPickerModel: ObservableObject {
...
struct ShareRecipeToChatSheet: View {
    let snapshot: SharedRecipeSnapshot
    var onSent: () -> Void

    @EnvironmentObject var app: AppState
    @Environment(\.dismiss) private var dismiss
    @StateObject private var model = ShareRecipeToChatPickerModel()
        ...
        if model.loading && model.rows.isEmpty {
            ...
            VStack(spacing: 8) {
                ...
                Text("No conversations yet")
                    .font(.headline)
                Text("Tap + to start a chat, then send this recipe.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .multilineTextAlignment(.center)
            }
    ...
    private func send(to conversationId: Int64) async {
        ...
        do {
            _ = try await ChatService.sendSharedRecipe(conversationId: conversationId, snapshot: snapshot)
            onSent()
            dismiss()
        } catch {
    ...
    private func sendToNew(profile: ChatProfile) async {
        do {
            ...
            _ = try await ChatService.sendSharedRecipe(conversationId: cid, snapshot: snapshot)
            await MainActor.run {
                showNewChat = false
                onSent()
                dismiss()
            ...
            await MainActor.run {
                sendError = ShareRecipeToChatPickerModel.friendlyStartError(error)
            }
        }
    }
}
