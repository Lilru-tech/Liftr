...
@Composable
fun ShareIngredientToChatSheetContent(
            ...
            when {
                ...
                error != null && rows.isEmpty() -> {
                    Column(
                        ...
                        Text("Retry", Modifier.clickable { reload() }, color = MaterialTheme.colorScheme.primary)
                    }
                }
                rows.isEmpty() -> {
                    Column(
                                ...
                                Modifier
                                    ...
                                    .clickable(enabled = !busy) {
                                        scope.launch {
                                            sendBusyId = row.id
                                            runCatching {
                                                repo.sendSharedIngredient(row.id, snapshot)
                                                onDone()
                                            }.onFailure { e ->
                                                error = e.message
                                            }
                ...
                onPick = { profile ->
                    scope.launch {
                        runCatching {
                            ...
                            repo.sendSharedIngredient(cid, snapshot)
                            showNewChat = false
                            onDone()
                        }.onFailure { e ->
                            error = e.message
