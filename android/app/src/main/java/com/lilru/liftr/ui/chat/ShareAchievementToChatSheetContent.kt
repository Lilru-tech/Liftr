...
@Composable
fun ShareAchievementToChatSheetContent(
    ...
    Box(modifier = modifier.fillMaxSize()) {
        Scaffold(
            ...
            topBar = {
                TopAppBar(
                    title = { Text(stringResource(R.string.share_achievement_title)) },
                    navigationIcon = {
            ...
            when {
                ...
                rows.isEmpty() -> {
                    Column(
                        ...
                        Text(stringResource(R.string.share_achievement_no_conversations))
                                ...
                                Modifier
                                    ...
                                    .clickable(enabled = !busy) {
                                        scope.launch {
                                            sendBusyId = row.id
                                            runCatching {
                                                repo.sendAchievementShare(row.id, snapshot)
                                                onDone()
                                            }.onFailure { e ->
                                                error = e.message
                                            }
                ...
                onPick = { profile ->
                    scope.launch {
                        runCatching {
                            ...
                            repo.sendAchievementShare(cid, snapshot)
                            showNewChat = false
                            onDone()
                        }.onFailure { e ->
