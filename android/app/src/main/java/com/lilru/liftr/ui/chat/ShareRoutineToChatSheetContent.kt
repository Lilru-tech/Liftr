...
@Composable
fun ShareRoutineToChatSheetContent(
    ...
    Box(modifier = modifier.fillMaxSize()) {
        Scaffold(
            ...
            topBar = {
                TopAppBar(
                    title = { Text(stringResource(R.string.share_routine_title)) },
                    navigationIcon = {
            ...
            when {
                ...
                rows.isEmpty() -> {
                    Column(
                        ...
                        Text(stringResource(R.string.share_routine_no_conversations))
                                ...
                                Modifier
                                    ...
                                    .clickable(enabled = !busy) {
                                        scope.launch {
                                            sendBusyId = row.id
                                            runCatching {
                                                repo.sendRoutineShare(row.id, snapshot)
                                                onDone()
                                            }.onFailure { e ->
                                                error = e.message
                                            }
                ...
                onPick = { profile ->
                    scope.launch {
                        runCatching {
                            ...
                            repo.sendRoutineShare(cid, snapshot)
                            showNewChat = false
                            onDone()
                        }.onFailure { e ->
...
private fun friendlyStartError(e: Throwable): String {
